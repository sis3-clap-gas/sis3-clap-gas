...
router.get('/agregarpersonas', (req, res) => {
  const query = "SELECT * FROM calles";
  connection.query(query, (err, row) => {
    res.render('persons', {
      calle: row
...
router.get('/familiaporcalle/edit/:id', auth.protectRouteAdmin, (req, res) => {
  const { id } = req.params;
  const queryFamilia = `
    SELECT f.*, p1.nombre AS jefe_nombre, p1.apellido AS jefe_apellido, 
           p2.nombre AS lider_nombre, p2.apellido AS lider_apellido 
    FROM familias f
    INNER JOIN personas p1 ON f.persona_id = p1.persona_id
    INNER JOIN lidercalle_calles lc ON f.liderdecalle_id = lc.id
    INNER JOIN personas p2 ON lc.usuario_id = p2.persona_id
    WHERE f.familia_id = ?
  `;
...
router.get('/admin_panel', auth.protectRouteAdmin, (req, res) => {
  const query = "SELECT p.persona_id AS id, p.nombre AS nombre, p.apellido AS apellido, p.telefono AS telefono, u.cédula AS cedula, u.rol AS rol FROM usuarios u INNER JOIN personas p ON u.persona_id = p.persona_id;"
  connection.query(query, (err, row) => {
...
router.get('/calles_panel', auth.protectRouteAdmin, (req, res) => {
  const query = "SELECT f.familia_id as id, p.nombre, p.apellido, p.direccion, c.nombre_calle, f.nombre_familia, f.cantidad_miembros FROM personas p INNER JOIN calles c ON c.calle_id = p.calle_id INNER JOIN familias f ON f.persona_id = p.persona_id;";
...
router.get('/pagos_panel', auth.protectRouteAdmin, (req, res) => {
  ...
  c.nombre_calle AS nombre_calle,  -- Nombre de la calle desde la tabla calles
  p1.direccion AS direccion_jefe,  -- Dirección del jefe de hogar desde la tabla personas
  p.fecha, 
  p.cantidad, 
  p.precio_total 
FROM entregas e 
...
INNER JOIN lidercalle_calles lc ON f.liderdecalle_id = lc.id 
INNER JOIN calles c ON lc.calle_id = c.calle_id  -- Relación con la tabla calles para obtener el nombre
INNER JOIN usuarios u ON lc.usuario_id = u.usuario_id 
INNER JOIN personas p2 ON u.persona_id = p2.persona_id  -- Líder de calle
INNER JOIN pagos p ON e.pago_id = p.entrega_id;
  `
  connection.query(query, (err, row) => {
...
router.get('/aprobar/:id', (req, res) => {
  ...
  connection.query(query1, [entregaId], (err, row) => {
    ...
    const inventarioId = row[0].inventario_id;  // Renombrado para evitar conflicto
        ...
        connection.query(queryUpdateEntrega, [aprobado, entregaId], (err, row1) => {
          ...
          connection.query(query5, [persona_id], (err, row3) => {
            ...
            connection.query(query4, [persona_id, mensaje, formattedDateTime], (err, row2) => {
              ...
              // Redirigir al panel de pagos
              res.redirect('/pagos_panel');
            });
          });
        });
      });
